...
import { gridExpandedSortedRowIdsSelector, gridVisibleColumnDefinitionsSelector, isAutogeneratedRow, useGridApiRef } from '@mui/x-data-grid';
...
function AccountTab(props) {
    ...
    const {receivableAccountsCount,receivableAccountsParams,receivableAccountsLoading} = useSelector((store) => store.account);
    const {payableAccountsCount,payableAccountsParams,payableAccountsLoading} = useSelector((store) => store.account);
    ...
    const receivableAccountsColumns = [
        { field: 'partner', headerName: 'Partner', flex: 6, editable: true, renderCell: (params) => (
                <Link
                to={`/accounts/update/${params.row.uuid}/`}
