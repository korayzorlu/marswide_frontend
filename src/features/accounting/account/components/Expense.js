...
import { fetchAccounts, fetchSalesAccounts, setAccountsLoading, setAccountsParams } from '../../../../store/slices/accounting/accountSlice';
...
function Expense() {
    ...
    const fetchData = async () => {
        const response = await dispatch(fetchAccounts({activeCompany,type:"expense",params:accountsParams})).unwrap();
        setData(response);
    }
    ...
    return (
        <Stack spacing={0}>
            ...
            finalEvent={() => {dispatch(fetchAccounts({activeCompany,type:"expense"}));dispatch(setAccountsLoading(false));}}
            />
        </Stack>
    )
}
